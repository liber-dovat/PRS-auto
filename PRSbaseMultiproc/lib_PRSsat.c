...
int procesar_NetCDF_VIS_gri(double ** FRmat, int ** MSKmat, int ** CNT1mat, int ** CNT2mat, int *tag,
	...
	int		h1, Si, Sj, St, Band, yea, doy, hra, min, sec, ste, kste, OK;
	...
	*tag = 0;
	...
	if (Band == 1){ // CANAL VISIBLE, PROCESO
  	...
  	if(DEBUG){printf("CANAL VISIBLE, PROCESO\n");}

		// printf("CANAL VISIBLE, PROCESO\n");
		
  	if(DEBUG){printf("calculo_solar_diario\n");}

	 	// calculo solar diario
	 	calculo_solar_diario(yea, doy, &Fn, &DELTArad, &EcTmin);

  	if(DEBUG){printf("procesar_VIS_gri\n");}

	 	// proceso imagen
	 	procesar_VIS_gri((*FRmat), (*MSKmat), (*CNT1mat), (*CNT2mat), &*tag, &fracMK,
		...
		// GUARDAR IMAGEN
  	if(DEBUG){printf("guardar_imagen_VIS\n");}
	 	guardar_imagen_VIS(RUTAsal, Ct, yea, doy, hra, min, sec, 
	 	 	(*FRmat), (*MSKmat), *tag, fracMK, Band);

	 	// GUARDAR IMAGEN
  	if(DEBUG){printf("guardar_imagen_int\n");}
	...
	// LIBERO MEMORIA
	if(DEBUG){printf("LIBERO MEMORIA\n");}
...
int procesar_NetCDF_IRB_gri(double ** TXmat,
	...
	*tag = 0;
	...
	if(DEBUG){printf("OPENING: %s :: [%04d, %04d] = %09d :: Banda = [%02d] :: Fecha = [%04d-%03d] :: Hora = [%02dhs-%02dmin-%02dsec] :: Satelite = [GOES%02d]\n",
		FileName, Si, Sj, St, Band, yea, doy, hra, min, sec, ste);}
...
int procesar_VIS_gri(double * FRmat, int * MSKmat,
			...
			if ((lon >= (LONmin - hLONcel))&&(lon <= (LONmax + hLONcel))){
				...
				if ((mI<=mS)&&(mI<Ci)&&(mS>=0)){
					if ((nI<=nS)&&(nI<Cj)&&(nS>=0)){
						...
						if ((Braw > 0)&&(cosz>0)){
							calculo_productos_VIS(Braw, Fn, fc, CALvis_Xspace, CALvis_M, CALvis_K, &fr);
							mk = 1;
						}
						if (cosz<=0){
							mk = 1;
	...
	if(DEBUG){printf("sumaMK: %d\n", sumaMK);}
	if(DEBUG){printf("Ct:     %d\n", Ct);}
...
int calculo_productos_VIS(int Braw, double Fn, double fc,
	double CALvis_Xspace, double CALvis_M, double CALvis_K, double *fr){
...
int guardar_imagen_VIS(char RUTAsal[CMAXstr], int Ct,
	int yea, int doy, int hra, int min, int sec, 
	double * FRmat, int * MKmat, int tag, double fracMK, int Band){
  ...
  if(DEBUG){printf("generar_strings_temporales\n");}
  ...
  if(DEBUG){printf("tag: %d\n", tag);}
	...
	// GUARDO IMPAINTING
 	if ((tag == 3)||(tag == 4)||(tag == 5)){
	  // printf("GUARDO IMPAINTING\n");	
	  if(DEBUG){printf("GUARDO IMPAINTING");}
...
int guardar_tag(char RUTAsal[CMAXstr], char strTMP[COUTstr], char strYEA[4], char strDOY[3], char strHRA[2],
  ...
  if(DEBUG){printf("guardar_tag fopen: %s\n", RUTA_TG);}
...
int guardar_imagen_int(char RUTAsal[CMAXstr], int Ct,
	...
	// RUTA
	strcpy(RUTA, RUTAsal); strcat(RUTA, "/B"); strcat(RUTA, strBANDA2); strcat(RUTA, "-CNT/"); strcat(RUTA, strYEA); strcat(RUTA, "/");
	strcat(RUTA, strTMP); strcat(RUTA, "."); strcat(RUTA, tipo);
	
	if(DEBUG){printf("RUTA ARCHIVO: %s\n", RUTA);}
