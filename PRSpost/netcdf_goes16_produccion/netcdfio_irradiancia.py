...
def getNameChannel(file):
  t_coverage = repr(file.getncattr('time_coverage_end'))
  # print t_coverage

  ds_name = repr(file.getncattr('dataset_name'))
  # print ds_name
...
def netcdf2png(url, colormapPath, colormapName, dirDest, lat_name, lon_name, data_name, geos=False):
  ...
  channel, name = getNameChannel(nc_fid)
  ...
  if data_name == 'CMI' or data_name == 'Rad':
    ...
    # sat_h -= 10000 # chr parche para ajustar la imagen
    ...
    if geos:
      ...
      if channel == 'C01' or channel == 'C03' or channel == 'C05':
        X = X[6000:10000]
        Y = Y[6000:10000]
        ...
        X = X[7000:9500:16]
        Y = Y[7000:9500:16]
        data = data[7000:9500:16, 7000:9500:16]
      else:
        X = X[3000:4800]
        Y = Y[3000:4800]
        data = data[3000:4800, 3000:4800]
    ...
    #   X = X[6000:10000:8]
    #   Y = Y[6000:10000:8]
    #   data = data[6000:10000:8, 6000:10000:8]
    # elif geos:
    #   X = X[::4]
    #   Y = Y[::4]
    #   data = data[::4, ::4]
  ...
  if geos: # proyecto toda la foto completa de geo estacionario
    ...
    print "amin(XX) " + str(numpy.amin(XX))
    print "amin(YY) " + str(numpy.amin(YY))
    ...
    X = map(lambda x: x+7000, X) # incremento X
    Y = map(lambda y: y+2500, Y) # incremento Y
    ...
    print "amin(X) " + str(numpy.amin(X))
    print "amin(Y) " + str(numpy.amin(Y))
  ...
  print "amin(data) " + str(numpy.amin(data))
  print "amax(data) " + str(numpy.amax(data))
  ...
  ticks       = [numpy.floor(vmin), numpy.floor(vmax)]
  ticksLabels = ticks
